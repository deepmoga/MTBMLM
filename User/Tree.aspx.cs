   ...
   public DataTable dt21 = new DataTable();
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            {
                string check = Common.Get(objsql.GetSingleValue("select regno from tblviewer where regno='" + Session["user"] + "'"));
                if (check != "")
                {
                    Panel1.Visible = true;
                }
                Tree(Session["user"].ToString());
            }
        ...
        #endregion
        {
            #region LeftNode
            {
                ...
                if (LinkButton1.Text != "")
                {
                    ...
                    Boolean status =Convert.ToBoolean(Common.Get(objsql.GetSingleValue("select active from usersnew where regno='" + LinkButton1.Text + "'")));
                    if (status == true)
                    {
        ...
        }
        {
            #region RightNode
            {
                ...
                if (LinkButton4.Text != "")
                {
                    ...
                    Boolean status = Convert.ToBoolean(Common.Get(objsql.GetSingleValue("select active from usersnew where regno='" + LinkButton4.Text + "'")));
                    if (status == true)
                    {
        ...
        }
        {
            ...
            {
                {
                    ...
                    if (LinkButton2.Text != "")
                    {
                        ...
                        Boolean status = Convert.ToBoolean(Common.Get(objsql.GetSingleValue("select active from usersnew where regno='" + LinkButton2.Text + "'")));
                        if (status == true)
                        {
            ...
            #region LeftNode-Right
            {
                ...
                }
                {
                    ...
                    if (LinkButton3.Text != "")
                    {
                        Boolean status = Convert.ToBoolean(Common.Get(objsql.GetSingleValue("select active from usersnew where regno='" + LinkButton3.Text + "'")));
                        if (status == true)
                        {
            ...
            {
                {
                    ...
                    if (LinkButton5.Text != "")
                    {
                        ...
                        Boolean status = Convert.ToBoolean(Common.Get(objsql.GetSingleValue("select active from usersnew where regno='" + LinkButton5.Text + "'")));
                        if (status == true)
                        {
            ...
            {
                {
                    ...
                    if (LinkButton6.Text != "")
                    {
                        ...
                        Boolean status = Convert.ToBoolean(Common.Get(objsql.GetSingleValue("select active from usersnew where regno='" + LinkButton6.Text + "'")));
                        if (status == true)
                        {
    ...
    protected void btnseacrh_Click(object sender, EventArgs e)
    {
        using (SqlConnection con = new SqlConnection(constring))
        {
            ...
            using (SqlCommand cmd = new SqlCommand("VAL_DOWNLINE", con))
            {
                ...
                cmd.Parameters.AddWithValue("@PID", Session["user"]);           // sponser id
                cmd.Parameters.AddWithValue("@ID", txtreg.Text);                            // node
                ...
                {
                    ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('Sorry Invalid DownLine')", true);
                }
